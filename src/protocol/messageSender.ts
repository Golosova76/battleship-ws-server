// функция отправки сообщения получателям по connectionId
...
export function sendRoomMessage(
    responseTargetConnectionIds: string[],
    responseMessage: RoomResponseMessage
): void {
...
export function sendBroadcastMessage(responseMessage: BroadcastResponseMessage): void {
    ...
    const allConnectionIds = activeConnectionContexts.map(
        (connectionContext) => connectionContext.connectionId
    );
